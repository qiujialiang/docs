	...
	while (getline(in, str)) {
		...
		if (tmp[0] == name) {
			...
			if (FindStation(Map, st) == 0) {
				...
				for (int i = 5; i < tmp.size(); i++) {
					if (Map.Stations[atoi(tmp[i].c_str())]->StationName == des) {
						tmp.insert(tmp.begin()+i,to_string(FindStation(Map, st)-1));
						break;
					}
				}
			}
...
void restore(BusMap&Map) { //还原初始化文件
	ifstream f1("save_routes.txt");
	ifstream f2("save_stations.txt");
	remove("routes.txt");
	remove("stations.txt");
	if (f1.good() && f2.good()) {
		ofstream in1("routes.txt");
